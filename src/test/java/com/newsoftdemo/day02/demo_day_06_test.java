...
import com.newsoftdemo.day02.config.ConfigurationRoot2;
 ...
 * @className（类名称）: demo_day_06_test
 * @author（创建人）: 冯孝刚
 * @createDate（创建时间）: 2022-08-07 13:33
 * @version（版本）: v1.0
 */
public class demo_day_06_test {
    @Test
    ...
    public void testFindAllAccount2() {
        ApplicationContext ac = new AnnotationConfigApplicationContext(ConfigurationRoot2.class);
        IAccount3Service as = ac.getBean("accountService04Impl", IAccount3Service.class);
        List<Account> acs = as.findAllAccount();
        if (null == acs || 0 == acs.size()) {
            System.out.println("没有找到数据");
